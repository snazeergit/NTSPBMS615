	...
	public void run(String... args) throws Exception {
		...
		/*	template.send("topic1", new MessageCreator() {
				...
				public Message createMessage(Session session) throws JMSException {
					Message message = session.createTextMessage("from Publisher " + new Date());
					return message;
				}
			});*/
		
		//LMBDA based Anonymous inner class for MessageCreator interface
		/*template.send("topic1",session->{
			Message message=session.createTextMessage("from Publisher:: "+new Date());
			return message;
		});*/
		
		//LMBDA based Anonymous inner class for MessageCreator interface
		template.send("topic1",session->	session.createTextMessage("from Sender:: "+new Date()));
		
		System.out.println("From Publisher :: The message is sent");
	}
